...
### Phase 1: Identity Verification (Humanity Protocol)
To access the Creator Portal and unlock the potential of platform rewards (up to a 2x multiplier), the creator must first prove their unique human identity.

1.  The Creator lands on the WebPayback Dashboard.
2.  Clicks on **"Login with Humanity"**.
3.  The WebPayback frontend generates an OAuth URL via the `@humanity-org/react-sdk` and redirects the user to the official Humanity Protocol portal.
4.  **The Humanity Gateway:**
    *   *If already registered:* The user authorizes WebPayback to read their *Humanity Score* and identity data.
    *   *If new user:* The Humanity portal prompts the user to download the official mobile app, scan their palm (generating a biometric *zk-proof*), and create their Human ID.
5.  Once completed, Humanity Protocol **automatically redirects** the user back to the WebPayback Dashboard.
6.  The WebPayback backend verifies the session, retrieves the *Humanity Score*, and creates the user profile. The reward multiplier is now active, and the Creator Portal is unlocked.

### Phase 2: Wallet Creation & Connection (Privy)
Once verified as a human, the creator needs a secure place to receive their earnings. The goal is to eliminate cognitive barriers associated with complex seed phrases while offering flexibility.

1.  Inside the Creator Portal, the user navigates to the **"Wallet & Withdrawals"** tab.
2.  If no wallet is connected, they click **"Create your Wallet"**.
3.  The **Privy** authentication module opens. The user can log in via Email, Google, or connect an existing external wallet (like MetaMask).
4.  **Web3 Integration:** In the background, if using email/social login, Privy instantly generates a "Smart Wallet" (Embedded Wallet) linked to that account using advanced cryptography (Shamir's Secret Sharing). 
5.  The user now owns a real, non-custodial wallet on the Humanity Protocol network but does not need to manage private keys manually, perfectly bridging the gap between Web2 UX and Web3 security.
...
2.  The **Citation Reward Engine** (Backend) calculates the base reward (e.g., 10 WPT).
3.  The engine queries the database to verify the creator's Humanity status and biometric score.
4.  **The Biometric Multiplier ("No Palm Scan, Base Yield"):** The base reward is dynamically multiplied according to the user's score. If a user has not scanned their palm via the Humanity App (Score = 0 or unverified), they receive only the base 1x yield. Conversely, a fully verified "Elite Human" (Score 95+) receives a 2x multiplier (e.g., 10 WPT becomes 20 WPT-HUMAN). This ensures bot-farming remains economically unviable.
5.  The backend instructs the `WPTHumanRewards` Smart Contract to mint and transfer the tokens directly into the user's connected Privy wallet.
